...
meta <- within(meta, {
  geo_codes <- fread(file.path(path$wd, "doc", "dst_regions_kommunes.csv"), 
                     encoding = 'UTF-8')
  geo_codes <- geo_codes %>% 
...
for (i in seq_along(meta$geo_codes$KODE)) {
  ...
  if (current_level == 1) {
    ...
    # Landsdel level - store as NUTS3 and assign NUTS2
    ...
    # Municipality level - assign both NUTS2 and NUTS3 if not special case
    if (current_code > 11) {  # Exclude special codes <= 11
      meta$geo_codes$nuts2[i] <- nuts2_code
      meta$geo_codes$nuts2_label[i] <- nuts2_label
...
# Optional: Create separate municipality dataset
municipalities <- meta$geo_codes %>%
  filter(NIVEAU == 3, KODE > 11, nuts3 != "") %>%
